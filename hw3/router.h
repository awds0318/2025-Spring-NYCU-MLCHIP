...
#define FIFO_SIZE 3 // Each input port corresponds to a FIFO with a maximum of 3 flits.
        ...
        else
        {
            if (in_req[i].read()) // When a flit is passed in
            {
                ...
                if (in_flit_reg[i][33] == 1)  // header flit
                {
                    ...
                    if (!output_busy[curr_dst[i]] && input_fifo[i].size() == 0) // If the direction is idle, the flit is allowed to be forwarded
                    {
        ...
        else
        {
            ...
            // But it does not mean that there must be data in FIFO that can be sent immediately!
            {
                ...
                if (!fifo.empty())  // There is data to send
                {
                    ...
                    else if (fifo.front()[33] == 1) // When head flit is in progress, out_req is too late to give 1, but there is still data to be sent
                    {
                        out_flit[i].write(fifo.front());
                        out_req[i].write(1);
                    }

...
SC_MODULE( Router ) {
    ...
    sc_out<sc_lv<2+32*FLIT_GROUP> > out_flit[5];
    sc_out<bool>       out_req[5];
    sc_in <bool>       in_ack[5];

    sc_in< sc_lv<2+32*FLIT_GROUP> > in_flit[5];
    sc_in<bool>        in_req[5];
    ...
    sc_lv <2+32*FLIT_GROUP> in_flit_reg[5];

    queue <sc_lv<2+32*FLIT_GROUP> > input_fifo[5];
...
void Router::run()
{
    ...
    for(int i = 0; i < 5; ++i)
    {
        ...
        else
        {
            if (in_req[i].read()) // When a flit is passed in
            {
                ...
                if (in_flit_reg[i].range((2+32*FLIT_GROUP)-1, (2+32*FLIT_GROUP)-2) == "10")  // header flit
                {
                    dst_id[i] = in_flit_reg[i].range((2+32*FLIT_GROUP)-7, (2+32*FLIT_GROUP)-10).to_uint();
                    ...
                    if (!output_busy[curr_dst[i]] && input_fifo[i].empty()) // If the direction is idle, the flit is allowed to be forwarded
                    {
                        ...
                        cout << "[Info] Router_" << router_id << " received a header flit from input port " << i << " to output port " << curr_dst[i] << endl;
    ...
    for(int i = 0; i < 5; ++i)
    {
        ...
        else
        {
            ...
            {
                queue<sc_lv<2+32*FLIT_GROUP> >& fifo = input_fifo[occupy_by_input[i]];
                ...
                if (!fifo.empty())  // There is data to send
                {
                    if (out_req[i].read() && in_ack[i].read())
                    {
                        ...
                        if (fifo.front().range((2+32*FLIT_GROUP)-1, (2+32*FLIT_GROUP)-2) == "01") // Transmission completed
                        {
                            // cout << "[Warning] Router_" << router_id << " finished sending a packet fragment." << endl;
                            fifo.pop();
                            output_busy[i] = false;
                            occupy_by_input[i] = i;
                            out_flit[i].write(0);

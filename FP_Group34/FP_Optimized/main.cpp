...
int sc_main(int argc, char *argv[])
{
    ...
    // Controller
    sc_signal<int>        ARADDR;         // '0' means input data
    ...
    sc_signal<float>      RDATA[FLIT_GROUP];
    sc_signal<bool>       RVALID;

    sc_signal<sc_lv<2+32*FLIT_GROUP> > flit_rx[16];      // The input channel
    ...
    // transmit
    sc_signal<sc_lv<2+32*FLIT_GROUP> > flit_tx[16];      // The output channel
    ...
    // Router: Input ports
    sc_signal<sc_lv<2+32*FLIT_GROUP> > in_flit_buf[16][5];
    ...
    m_rom.ARVALID(ARVALID);
    m_rom.ARREADY(ARREADY);
    m_rom.RREADY(RREADY);
    for (int i = 0; i < FLIT_GROUP; ++i)
    {
        m_rom.RDATA[i](RDATA[i]);
    }
    // m_rom.RDATA(RDATA);
    m_rom.RVALID(RVALID);

...
SC_MODULE(Controller)
{
    ...
    sc_out<bool>       RREADY;
    // from ROM
    sc_in<float>       RDATA[FLIT_GROUP];
    sc_in<bool>        RVALID;

    // to router0
    // sc_out<sc_lv<2018> > flit_tx;
    sc_out<sc_lv<2+32*FLIT_GROUP> > flit_tx;
    sc_out<bool>       req_tx;
    sc_in<bool>        ack_tx;

    // from router0
    sc_in<sc_lv<2+32*FLIT_GROUP> >  flit_rx;
    // sc_in<sc_lv<2018> >  flit_rx;
    sc_in<bool>        req_rx;
    sc_out<bool>       ack_rx;
    ...
    sc_buffer<sc_lv<34> > RDATA_reg[FLIT_GROUP];
...
void Controller::run()
{
    ...
    else
    {
        ...
        else if (cur_step == 17)
        {
            ...
            if(tx_queue.empty())
            {
                // cout << "[ERROR] tx_queue is not empty when sending header flit." << endl;
                // sc_stop();
        ...
        else
        {
            ...
            sc_lv<2+32*FLIT_GROUP> tmp_flit = 0;
            tmp_flit.range((2+32*FLIT_GROUP)-1, (2+32*FLIT_GROUP)-34) = send_flit; // header flit
            cout << "header flit: " << send_flit << endl;
        ...
        if (RVALID.read())
        {
            ...
            {
                RDATA_reg[i].write(float2lv(RDATA[i].read()));
            }
        }
        ...
        if (RVALID_reg.read())
        {
            ...
            for (int i = 0; i < FLIT_GROUP; ++i)
            {
                // RDATA_reg[i].write(0); // Clear the RDATA_reg
                current_group.push_back(RDATA_reg[i].read());
            }
            // tx_queue.push(send_flit);
            if (!RVALID.read())
            {
                // cout << "[INFO] Controller form the tail flit." << endl;
                // cout << "controller_cnt= "<< controller_cnt << endl;
                is_tail = true;
            }
            if (current_group.size() == FLIT_GROUP) 
            {
                // cout << "is_tail= " << is_tail << endl;
                sc_lv<2+32*FLIT_GROUP> wide_flit = 0;
                sc_lv<2> crtl_bits = (is_tail)? "01" : "00"; // Set the tail bit
                // cout << "crtl_bits= " << crtl_bits << endl;
                ...
                // cout << "[DEBUG] Push head flit, tx_queue.size() = " << tx_queue.size() << endl;

                current_group.clear();
                controller_cnt++;
                if (crtl_bits == "01") 
                {
                    // cout << "[Warning] Controller send the tail flit. controller_cnt= "<< controller_cnt << endl;
                    controller_cnt = 0; // Reset the controller count
                }

            }

            if (!current_group.empty() && !RVALID.read()) // If the current group is not empty and we are sending a tail flit
            {
                cout << "current_group size= " << current_group.size() << endl;
                cout << "is_tail= " << is_tail << endl;
                ...
                controller_cnt++;
                test_cnt++;
                // cout << "[Warning] Controller send the tail flit. controller_cnt= "<< controller_cnt  << "test_cnt = " << test_cnt << endl;
                is_tail = false; // Reset is_tail for the next flit
                controller_cnt = 0; // Reset the controller count
                // cout << "[INFO] Controller send " << test_cnt << " flits." << endl;
            }
        }
        ...
        {
            // cout << "[DEBUG] Pop flit: " << tx_queue.front().range(2017, 2016) << endl;
            tx_queue.pop();
            // cout << tx_queue.size() << " flits in tx_queue." << "tx_queue.empty()" <<tx_queue.empty()<< endl;
            // test_cnt++;
            // cout << "[INFO] Controller pop " << test_cnt << " flits." << endl;
        }
        ...
        if (req_rx.read() && ack_rx.read()) // When req_rx == 1 && ack_rx == 1, it means both parties are ready, and the flit can be successfully transmitted from the Router to the Core.
        {
            ...
            for (int i = 0; i < FLIT_GROUP; ++i) 
            {
                sc_lv<32> flit = recieve_flit.range((i + 1) * 32 - 1, i * 32);
                if (recieve_flit.range((2+32*FLIT_GROUP)-1, (2+32*FLIT_GROUP)-2) != "10" && flit != dummy) // exclude header
                    fc8_result.push_back(lv2float(flit));
            }
        ...
        // ---------- Output port control ----------
        if (!tx_queue.empty()) // If there are flits in the queue and we are not stopping sending
        {
            req_tx.write(1); // Request to send flit
            sc_lv<2> tmp = tx_queue.front().range((2+32*FLIT_GROUP)-1, (2+32*FLIT_GROUP)-1);
            flit_tx.write(tx_queue.front()); // Send the front flit in the queue
        }
        else
        {
            req_tx.write(0);
        ...
        // if (!tx_queue.empty())
            // cout << "[INFO] Controller send flit: " << tx_queue.front().range(2017, 2016) << endl;
        // if (!tx_queue.empty() && !stop_send)
            // cout << "[INFO] flit_tx send flit: " << tx_queue.front().range(2017, 2016) << endl;

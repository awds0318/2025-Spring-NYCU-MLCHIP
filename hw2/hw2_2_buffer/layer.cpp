            ...
            for (int o_y = 0; o_y < OUT_HEIGHT; ++o_y)
            {
                for (int o_x = 0; o_x < OUT_WIDTH; ++o_x)
                {
                        ...
                        for (int k_y = int(KERNEL_SIZE / 2) * (-1); k_y < int(KERNEL_SIZE / 2) + 1; ++k_y)
                        {
                            for (int k_x = int(KERNEL_SIZE / 2) * (-1); k_x < int(KERNEL_SIZE / 2) + 1; ++k_x)
                            {
                                ...
                                if (in_feature_y >= 0 && in_feature_y < IN_HEIGHT && in_feature_x >= 0 && in_feature_x < IN_WIDTH)
                                {
                                    ...
                                    sum += (in_feature[in_feature_idx] * weight[weight_idx]);
                                }
                            }
                        }
                    }
                    ...
                    if (result < 0)
                        ...
                        out_feature[out_feature_idx] = (double)result;
                }
            }
        }
        out_valid.write(1);
                ...
                for (int o_x = 0; o_x < OUT_SIZE; ++o_x)
                {
                    ...
                    for (int k_y = -1; k_y <= 1; ++k_y)
                    {
                        for (int k_x = -1; k_x <= 1; ++k_x)
                        {
                            ...
                            if (in_feature[tmp_idx] > tmp)
                                tmp = in_feature[tmp_idx];
                        }
                    }

                    out_feature[(o_c * OUT_SIZE * OUT_SIZE) + (o_y * OUT_SIZE) + o_x] = tmp;
                }
            }
        }
        out_valid.write(1);
...
void FC::run()
{
    ...
    if (in_valid.read() == 1)
    {
        ...
        for (int o_c = 0; o_c < OUT_CH; ++o_c)
        {
            ...
            for (int i_c = 0; i_c < IN_CH; ++i_c)
            {
                sum += (in_feature[i_c] * weight[(o_c * IN_CH) + i_c]);
            }
...
void SOFTMAX::run()
{
    ...
    if(in_valid.read() == 1)
    {
        ...
        for(int i = 0; i < SIZE; ++i)
            exp_sum += (double) exp((double)(in_feature[i].read()));

        for(int i = 0; i < SIZE; ++i)
        {
            tmp = (double)(in_feature[i].read());
            a = (double) exp(tmp);

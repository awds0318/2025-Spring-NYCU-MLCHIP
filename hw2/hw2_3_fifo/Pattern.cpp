...
void Pattern::run()
{
	...
	else if (out_valid.read() == 1)
	{
		...
		vector<double> output_softmax_tmp(1000);
		vector<double> output_linear_tmp(1000);

		for (int i = 0; i < 1000; ++i) 
		{
			output_softmax[i].read(output_softmax_tmp[i]);
			output_linear[i].read(output_linear_tmp[i]);
		}


		for (int i = 0; i < 1000; ++i) 
		{
        		indexed_values.push_back(make_pair(output_softmax_tmp[i], i));
		}
		...
		for (int i = 0; i < 100; ++i)
		{
			cout << right << setw(5) << indexed_values[i].second
				 << " | " << setw(8) << double(output_linear_tmp[indexed_values[i].second])
